...
const STEPS: Record<Locale, Array<{ n: string; title: string; detail: string }>> = {
  zh: [
    ...
    { n: "4", title: "执行", detail: "Core 创建并维护 Run；Runtime Worker 接单执行，其他模式由 Core 调用" },
  ...
  en: [
    ...
    { n: "4", title: "Execute", detail: "Core creates and maintains the Run; Runtime Worker accepts it, while Core invokes the other modes" },
  ...
  const copy =
    locale === "zh"
      ? {
          ...
          runtimeModes: "runtime · WebSocket 优先 / 长轮询回退",
          waiting: "已注册 · 等待调用",
          skills: "声明的 Skill",
