...
export function A2AConformancePanel({
  ...
  async function runStreamCheck() {
    ...
    try {
      ...
      setCheck("stream", { state: "fail", detail: err instanceof Error ? err.message : String(err) });
  ...
  async function runLongOnlineCheck() {
    ...
    try {
      ...
      setCheck("long-online", { state: "fail", detail: err instanceof Error ? err.message : String(err) });
      setSummary(err instanceof Error ? err.message : String(err));
    } finally {
      setBusy(false);
    }
