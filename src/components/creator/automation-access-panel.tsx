...
function buildSelfRegistrationPrompt(token: string, locale: Locale): string {
  ...
  if (locale === "en") {
    return [
      ...
      "3. Prefer direct_http for a reachable HTTPS endpoint. Use connection_mode=runtime when an official or third-party SDK Runtime Worker will actively connect to OpenLinker. Use mcp_server only to wrap an existing remote HTTP JSON-RPC / MCP tools/call endpoint as an Agent. Explicitly set visibility=private unless the human asked for public.",
      "4. Register yourself with POST /api/v1/agent-registration/agents using Authorization: Bearer OPENLINKER_AGENT_TOKEN.",
      "5. For runtime, choose the Go, server-only TypeScript, or async Python Runtime SDK. Configure OPENLINKER_URL, the enrolled Runtime Node identity, this Agent Token, mTLS material, transport=auto, a production durable RuntimeStore, and your RuntimeHandler. The SDK Worker owns discovery, Session lifecycle, assignment confirmation, lease renewal, cancellation, resume, transport switching, and durable Event/Result delivery. If an existing HTTP, command, Codex, or A2A backend cannot yet use an SDK handler directly, Agent Node Adapter may temporarily wrap the Go SDK Worker.",
  ...
  return [
    ...
    "3. 有可达 HTTPS endpoint 时优先选择 direct_http；由官方或第三方 SDK Runtime Worker 主动连接 OpenLinker 时使用 connection_mode=runtime。只有把已有远程 HTTP JSON-RPC / MCP tools/call 工具包装成 Agent 时才使用 mcp_server。除非人类明确要求 public，否则请显式设置 visibility=private。",
    "4. 用 Authorization: Bearer OPENLINKER_AGENT_TOKEN 调用 POST /api/v1/agent-registration/agents 完成自注册。",
    "5. 使用 runtime 时，选择 Go、仅服务端 TypeScript 或异步 Python Runtime SDK。配置 OPENLINKER_URL、已登记的 Runtime Node 身份、当前 Agent Token、mTLS 材料、transport=auto、生产环境的持久化 Runtime Store 和你的 Runtime Handler。SDK Worker 负责发现、Session 生命周期、任务确认、租约续期、取消、恢复、通道切换与 Event/Result 可靠提交。已有 HTTP、命令、Codex 或 A2A 服务暂时无法直接实现 SDK Handler 时，才用 Agent Node Adapter 包装 Go SDK Worker。",
