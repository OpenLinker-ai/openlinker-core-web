  ...
  const copy =
    locale === "zh"
      ? {
          ...
          tokenBoundarySuffix: "注册 Agent。详细接入步骤、Skill 映射规则和 runtime_pull 保活要求统一维护在",
      ...
      : {
          ...
          tokenBoundarySuffix: "to register an Agent. Detailed onboarding steps, Skill mapping, and runtime_pull keepalive rules live in",
...
function buildSelfRegistrationPrompt(token: string, locale: Locale): string {
  ...
  if (locale === "en") {
    return [
      ...
      "3. Prefer direct_http when you have a reachable HTTPS endpoint. Use mcp_server when you expose a remote HTTP JSON-RPC / MCP tools/call endpoint. Use runtime_pull only as the fallback for local/private-network/NAT Agents that cannot accept inbound calls. Explicitly set visibility=private unless the human asked for public.",
      "4. Register yourself with POST /api/v1/agent-registration/agents using Authorization: Bearer OPENLINKER_REGISTRATION_TOKEN.",
      "5. If using runtime_pull, immediately start heartbeat plus GET /api/v1/agent-runtime/runs/claim?wait=25. Respect Retry-After on 204 or 429. A claimed run must always end with POST /api/v1/agent-runtime/runs/{run_id}/result.",
  ...
  return [
    ...
    "3. 如果你有可达 HTTPS endpoint，优先选择 direct_http；如果你暴露的是远程 HTTP JSON-RPC / MCP tools/call 工具端点，选择 mcp_server；只有本地/内网/NAT 后面无法接收入站调用时，才把 runtime_pull 作为降级方案。除非人类明确要求 public，否则请显式设置 visibility=private。",
    "4. 用 Authorization: Bearer OPENLINKER_REGISTRATION_TOKEN 调用 POST /api/v1/agent-registration/agents 完成自注册。",
    "5. 如果使用 runtime_pull，注册后立即启动 heartbeat 和 GET /api/v1/agent-runtime/runs/claim?wait=25。遇到 204 或 429 必须遵守 Retry-After。只要领取了 run，就必须 POST /api/v1/agent-runtime/runs/{run_id}/result。",
