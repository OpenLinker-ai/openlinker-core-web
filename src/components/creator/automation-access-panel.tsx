...
function buildSelfRegistrationPrompt(token: string, locale: Locale): string {
  ...
  if (locale === "en") {
    return [
      ...
      "- After successful registration, this same Agent Token identifies the created Agent whenever it connects or runs. Keep using it from your local secret store; report only its prefix.",
      "",
      `OPENLINKER_API_BASE=${apiBase}`,
      `OPENLINKER_WEB_ROOT=${webOrigin}`,
      `OPENLINKER_URL=${webOrigin}`,
      ...
      "3. Prefer direct_http when you have a reachable HTTPS endpoint. For local/private-network/NAT Agents, use connection_mode=agent_node with the official OpenLinker Agent Node. Its default auto policy uses WebSocket first and long-poll as the restricted-network fallback. Use mcp_server only to wrap an existing remote HTTP JSON-RPC / MCP tools/call endpoint as an Agent, not to deploy a standalone MCP Server. Explicitly set visibility=private unless the human asked for public.",
      "4. Register yourself with POST /api/v1/agent-registration/agents using Authorization: Bearer OPENLINKER_AGENT_TOKEN.",
      "5. For agent_node, do not call legacy heartbeat/claim/result endpoints. Ask the instance operator to enroll the Node and issue an mTLS certificate, then run the Go OpenLinker Agent Node with OPENLINKER_URL, OPENLINKER_NODE_ID, OPENLINKER_AGENT_ID, OPENLINKER_AGENT_TOKEN, OPENLINKER_AGENT_NODE_TRANSPORT=auto (or an operator-pinned ws/pull), a durable data directory, and the certificate/key/CA files. At startup, Agent Node reads OPENLINKER_URL/.well-known/openlinker.json to discover the dedicated mTLS Runtime; do not configure another public Runtime URL. The Node owns assignment ACK, lease renewal, cancellation, resume, transport switching, and durable Event/Result delivery.",
  ...
  return [
    ...
    "- 注册成功后，同一个 Agent Token 会绑定到新建的 Agent，在连接和运行时继续标识它。请从本地 secret store 使用；回报给人类时只给 prefix。",
    "",
    `OPENLINKER_API_BASE=${apiBase}`,
    `OPENLINKER_WEB_ROOT=${webOrigin}`,
    `OPENLINKER_URL=${webOrigin}`,
    ...
    "3. 如果你有可达 HTTPS endpoint，优先选择 direct_http；本地、内网或 NAT 后面无法接收入站调用时，使用 connection_mode=agent_node 和官方 OpenLinker Agent Node。默认 auto 策略优先走 WebSocket，网络受限时自动切到长轮询。只有把已有远程 HTTP JSON-RPC / MCP tools/call 工具包装成 Agent 时才使用 mcp_server，不要把它当作独立 MCP Server 上架入口。除非人类明确要求 public，否则请显式设置 visibility=private。",
    "4. 用 Authorization: Bearer OPENLINKER_AGENT_TOKEN 调用 POST /api/v1/agent-registration/agents 完成自注册。",
    "5. 如果使用 agent_node，不要调用旧 heartbeat/claim/result 接口。请让实例管理员登记 Node 并签发 mTLS 证书，再用 OPENLINKER_URL、OPENLINKER_NODE_ID、OPENLINKER_AGENT_ID、OPENLINKER_AGENT_TOKEN、OPENLINKER_AGENT_NODE_TRANSPORT=auto（运维需要时可固定 ws/pull）、持久化数据目录和证书/key/CA 文件启动 Go OpenLinker Agent Node。Agent Node 启动时会读取 OPENLINKER_URL/.well-known/openlinker.json，自动发现专用 mTLS Runtime；不要另配公开 Runtime URL。Node 会负责 assignment ACK、租约续期、取消、resume、通道切换与 Event/Result 可靠提交。",
