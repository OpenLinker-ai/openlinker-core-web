...
function buildSelfRegistrationPrompt(token: string, locale: Locale): string {
  ...
  if (locale === "en") {
    return [
      ...
      "3. Prefer direct_http when you have a reachable HTTPS endpoint. For local/private-network/NAT Agents, use connection_mode=agent_node with the official OpenLinker Agent Node. Its default auto policy uses WebSocket first and Pull v2 as the restricted-network fallback. Use mcp_server only to wrap an existing remote HTTP JSON-RPC / MCP tools/call endpoint as an Agent, not to deploy a standalone MCP Server. Explicitly set visibility=private unless the human asked for public.",
      "4. Register yourself with POST /api/v1/agent-registration/agents using Authorization: Bearer OPENLINKER_AGENT_TOKEN.",
      "5. For agent_node, do not call legacy heartbeat/claim/result endpoints. Ask the instance operator to enroll the Node and issue an mTLS certificate, then run the Go OpenLinker Agent Node with OPENLINKER_CORE_V2_URL, OPENLINKER_NODE_ID, OPENLINKER_AGENT_ID, OPENLINKER_AGENT_TOKEN, OPENLINKER_AGENT_NODE_TRANSPORT=auto (or an operator-pinned ws/pull), a durable data directory, and the certificate/key/CA files. The Node owns assignment ACK, lease renewal, cancellation, resume, transport switching, and durable Event/Result delivery.",
  ...
  return [
    ...
    "3. 如果你有可达 HTTPS endpoint，优先选择 direct_http；本地、内网或 NAT 后面无法接收入站调用时，使用 connection_mode=agent_node 和官方 OpenLinker Agent Node。默认 auto 策略优先走 WebSocket，网络受限时自动切到 Pull v2。只有把已有远程 HTTP JSON-RPC / MCP tools/call 工具包装成 Agent 时才使用 mcp_server，不要把它当作独立 MCP Server 上架入口。除非人类明确要求 public，否则请显式设置 visibility=private。",
    "4. 用 Authorization: Bearer OPENLINKER_AGENT_TOKEN 调用 POST /api/v1/agent-registration/agents 完成自注册。",
    "5. 如果使用 agent_node，不要调用旧 heartbeat/claim/result 接口。请让实例管理员登记 Node 并签发 mTLS 证书，再用 OPENLINKER_CORE_V2_URL、OPENLINKER_NODE_ID、OPENLINKER_AGENT_ID、OPENLINKER_AGENT_TOKEN、OPENLINKER_AGENT_NODE_TRANSPORT=auto（运维需要时可固定 ws/pull）、持久化数据目录和证书/key/CA 文件启动 Go OpenLinker Agent Node。Node 会负责 assignment ACK、租约续期、取消、resume、通道切换与 Event/Result 可靠提交。",
