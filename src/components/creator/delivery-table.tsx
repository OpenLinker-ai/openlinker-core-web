...
export function DeliveryTable({ items, locale = "zh" }: Props) {
  const copy =
    locale === "zh"
      ? {
          empty: "暂无投递或回调记录。配置通知投递目标或任务回调后，每次触发都会在这里留下一条记录。",
          time: "时间",
          status: "状态",
          response: "响应码",
          attempts: "尝试",
      ...
      : {
          empty: "No delivery or callback records yet. After a notification delivery target or task callback is configured, each attempt leaves a record here.",
          time: "Time",
          status: "Status",
          response: "Response",
          attempts: "Attempts",
