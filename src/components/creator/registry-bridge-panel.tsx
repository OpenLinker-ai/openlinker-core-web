  ...
  const copy =
    locale === "zh"
      ? {
          ...
          body: "先创建 Registry Node，再显式把某个 Agent 链接成 Registry Listing。后续路由调用会沿用这个节点配置。",
          empty: "还没有 Registry Listing。创建后，其他 Registry 节点才能发现并桥接这个 Agent。",
      ...
      : {
          ...
          body: "Create a Registry Node first, then explicitly link an Agent as a Registry Listing. Subsequent routed calls reuse this node configuration.",
          empty: "No Registry Listings yet. Create one so other Registry nodes can discover and bridge this Agent.",
