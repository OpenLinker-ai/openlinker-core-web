...
export function RegistryBridgePanel({
  ...
  const [nodeName, setNodeName] = useState("Local Bridge Node");
  ...
  const copy =
    locale === "zh"
      ? {
          ...
          body: "先创建 Registry Node，再显式把某个 Agent 链接成 Registry Listing。后续 pull proxy run 会复用同一个节点身份和 payload policy。",
          ...
          nodeCreated: "Registry Node 已创建，secret 只显示这一次",
  ...
  const reload = async () => {
    const [nodeData, listingData] = await Promise.all([
      apiFetch<{ items: RegistryNode[] }>("/api/v1/registry-node/nodes"),
  ...
  const createNode = async () => {
    ...
    setSubmitting(true);
    try {
      const node = await apiFetch<RegistryNode>("/api/v1/registry-node/link", {
        method: "POST",
        body: {
          node_name: name,
      ...
      toast.error(err instanceof ApiError ? err.message : copy.nodeCreateFailed);
    } finally {
      setSubmitting(false);
    }
  };
  ...
  const createListing = async () => {
    ...
    try {
      ...
      toast.error(err instanceof ApiError ? err.message : copy.listingCreateFailed);
    } finally {
      setSubmitting(false);
    }
  };
  ...
  return (
    <div className="ol-panel overflow-hidden">
      <div className="ol-panel-head">
        <strong>Registry Bridge / Registry Listing</strong>
