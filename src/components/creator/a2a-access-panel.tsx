...
export function A2AAccessPanel({
  ...
  const copy =
    locale === "zh"
      ? {
          ...
          creating: "创建中",
          create: "创建",
          tokenHint: "凭证明文仅显示一次，请配置到 Agent 运行环境。它会绑定当前 Agent，底层使用统一访问令牌格式。",
          copied: "已复制",
          copyToken: "复制凭证",
          revoke: "撤销",
      ...
      : {
          ...
          creating: "Creating",
          create: "Create",
          tokenHint: "The credential plaintext is shown only once. Configure it in the Agent runtime environment. It is bound to this Agent and uses the unified access-token format.",
          copied: "Copied",
  ...
  useEffect(() => {
    let active = true;
    Promise.all([
      apiFetch<{ items?: RuntimeToken[] }>(
        `/api/v1/creator/agents/${agentId}/runtime-tokens`,
      ),
      ...
      .catch((err) => {
        if (active) toast.error(errorMessage(err, copy.loadFailed));
      })
      .finally(() => {
  ...
  }, [agentId, apiFetch, copy.loadFailed]);
  ...
  const updatePolicy = async (value: CallableBy) => {
    ...
    try {
      ...
      toast.error(errorMessage(err, copy.policyFailed));
    } finally {
  ...
  const createToken = async () => {
    ...
    try {
      const response = await apiFetch<RuntimeToken>(
        `/api/v1/creator/agents/${agentId}/runtime-tokens`,
      ...
      toast.error(errorMessage(err, copy.tokenCreateFailed));
    } finally {
  ...
  const revokeToken = async (tokenId: string) => {
    ...
    try {
      await apiFetch(`/api/v1/creator/runtime-tokens/${tokenId}`, { method: "DELETE" });
      setTokens((items) => items.filter((token) => token.id !== tokenId));
      toast.success(copy.tokenRevoked);
    } catch (err) {
      toast.error(errorMessage(err, copy.tokenRevokeFailed));
    } finally {
      setRevokingId("");
    }
  };
