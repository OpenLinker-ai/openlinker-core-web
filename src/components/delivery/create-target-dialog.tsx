...
const TYPE_HINTS: Record<"webhook" | "slack", { label: Record<Locale, string>; placeholder: string; hint: Record<Locale, string> }> = {
  webhook: {
    ...
    hint: {
      zh: "Run 完成后平台 POST 投递 payload，并在 X-OpenLinker-Signature 中带 HMAC-SHA256 签名。",
      en: "After a Run completes, OpenLinker POSTs the payload with an HMAC-SHA256 signature in X-OpenLinker-Signature.",
    },
  },
  slack: {
  ...
  const copy =
    locale === "zh"
      ? {
          ...
          title: "新增投递目标",
          desc: "配置一个目标后，可在 /run/<id> 点投递，或开启默认在 Run 完成后自动投递。",
          name: "名称",
          type: "类型",
          default: "设为默认（Run 完成后自动投递到这里）",
          cancel: "取消",
          creating: "创建中…",
          create: "创建",
      ...
      : {
          ...
          desc: "After configuring a target, you can deliver from /run/<id> or mark it as the default for automatic delivery when a Run completes.",
          name: "Name",
          type: "Type",
