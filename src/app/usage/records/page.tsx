...
export default async function UsageRecordsPage({
  ...
  if (!session) {
    const callbackUrl = pathWithSearchParams("/usage/records", sp);
    redirect(`/login?callbackUrl=${encodeURIComponent(callbackUrl)}`);
  }

  const locale = await getLocale();
  const page = Math.max(1, Number(firstSearchParam(sp.page) ?? "1") || 1);
  const callView = normalizeCallView(firstSearchParam(sp.call_view));
  const query = (firstSearchParam(sp.q) ?? "").trim().slice(0, 200);
  const sort = normalizeCallSort(firstSearchParam(sp.sort));
  const status = normalizeCallStatus(firstSearchParam(sp.status));
  const source = normalizeCallSource(firstSearchParam(sp.source));
  const relation = normalizeCallRelation(firstSearchParam(sp.relation));
  const size = 20;
  const copy =
    locale === "zh"
      ? {
