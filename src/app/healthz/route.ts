...
async function loadHealth(request: Request) {
  return fetch(`${getApiBaseUrlForRequest(request)}/healthz`, {
    method: "GET",
...
export async function GET(request: Request) {
  ...
  try {
    const upstream = await loadHealth(request);
    return proxyJSON(upstream, await upstream.text());
  } catch {
    return upstreamUnavailableResponse("/healthz");
  }
}
