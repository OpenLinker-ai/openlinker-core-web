  ...
  const copy =
    locale === "zh"
      ? {
          ...
          heading: `${"AGENT"} · 被调用记录`,
          lead: "这里展示这个 Agent 被用户、访问凭证或 MCP 工具触发的运行记录。自注册 Agent 会绑定到生成 Agent 接入凭证的 creator 用户，因此也会出现在同一份列表里。",
          onboarding: "接入配置",
          back: "返回创作者中心",
          listTitle: "被调用记录",
      ...
      : {
          ...
          lead: "This shows runs triggered by users, access credentials, or MCP tools. Self-registered Agents are bound to the creator user that generated their Agent access credential, so those runs appear here too.",
