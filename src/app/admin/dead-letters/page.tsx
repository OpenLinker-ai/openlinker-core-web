...
export default async function AdminDeadLettersPage({
  ...
  const copy = locale === "zh"
    ? {
        title: "待人工处理的运行",
        lead: "这里保留已经用完自动重试次数的运行记录。原运行不会再次执行；选择重新运行时，系统会按当前 Agent、权限、配额和设置创建一条新运行。",
        total: "当前记录",
        replayed: "已重新运行",
        noData: "没有待人工处理的运行",
        ...
        lineage: "后续新运行",
        noReplay: "尚未重新运行",
        ownerAction: "运行所有者可从运行详情使用相同输入重新运行；管理员列表不会代替用户触发新的外部操作。",
        inspect: "查看技术详情",
        runId: "run_id",
    ...
    : {
        ...
        lead: "This view keeps runs that exhausted automatic retries. The original run never executes again; running it again creates a new run under current Agent, permission, quota, and settings checks.",
        total: "Current records",
        replayed: "Runs started again",
        ...
        created: "Added",
        lineage: "New runs created from this one",
        noReplay: "Not run again yet",
        ownerAction: "The run owner can run it again with the same input from Run detail. This admin list never triggers new external actions on the user's behalf.",
        inspect: "View technical details",
        runId: "run_id",
        deadLetterId: "dead_letter_id",
        finalAttempt: "final_attempt_id",
