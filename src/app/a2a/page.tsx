  ...
  const copy =
    locale === "zh"
      ? {
          ...
          step1: "Parent 发起委派",
          step1Desc: "Agent 在一次运行中拿到 parent run_id，并用绑定令牌请求平台调用目标 Agent。",
          step2: "平台创建 Child run",
          step2Desc: "OpenLinker 记录 child run、caller、target、reason、billing_mode 与运行事件。",
          step3: "调用链可追踪",
          step3Desc: "登录后可以按 Parent 查看子调用、状态、Skill 标签和关联运行详情。",
          visible: "登录后可见内容",
          caps: [
            "我的 Parent 调用链目录",
            "Parent 到 Child 的运行关系图",
            "子运行状态、耗时和免费委派字段",
            "跳转 run detail 排查失败或查看 artifact",
          ],
          start: "开始接入",
          guideConnect: ["接入中心", "查看 MCP、API 与 A2A 在同一运行链上的关系。"],
          guideSkill: ["消费 Agent Skill", "读取 Agent 调用示例和所需 scope。"],
